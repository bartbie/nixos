...
let
  ...
  makeProperty =
    ...
    in
    {
      ...
      map = if customMap != null then customMap else fn: prop: prop // { content = fn prop.content; };
    };
  ...
  stripEntries =
    ...
    x |> builtins.mapAttrs (n: v: if isEntry v then v.data else v);
  ...
  Dag = makeProperty {
    ...
    customMap =
      ...
      Dag.mk (builtins.mapAttrs fn content);
  };
  ...
  maybeConvert =
    ...
    if Entry.is value then
      value
    else
      entryAnywhere (if priority != null then mkOrder priority value else value);
    ...
    lib.mkOptionType {
      ...
      merge =
        ...
        |> builtins.map (
          ...
          {
            inherit file;
            value = if isDag value then value.content or { } else value;
          }
        )
    ...
    lib.mkOptionType {
      ...
      merge =
        ...
        let
          ...
          merged-metadata =
            defs
            |> (mapDefs (builtins.mapAttrs (_: v: if isEntry v then v else entryAnywhere v)))
          ...
          merged-data =
            ...
            |> mapDefs (
              let
                walk =
                  def:
                  if builtins.isAttrs def then
                    def
                    |> lib.mapAttrsRecursiveCond (as: !(isEntry as)) (
                      path: entry: if isEntry entry then walk entry.data else entry
                    )
                  else
                    def;
              in
        ...
        merged-metadata |> builtins.mapAttrs (n: v: v // { data = merged-data.${n}; }) |> mkDag;
    };
  ...
  submoduleTypeWith =
    ...
    submoduleDef:
    (if dag then dagSubmodule else types.submodule) {
  ...
  topoSort =
    let
      dagBefore =
        ...
        dag |> lib.filterAttrs (n: v: builtins.elem name v.before) |> builtins.attrNames;
      before = a: b: builtins.elem a.name b.after;
    in
    dag:
    assert isDagLike dag;
    let
      normalized =
        if isDagNormalized dag then
          dag.content or { }
        else if isDag dag then
          dag.content or { }
          |> builtins.mapAttrs (_: v: if v ? value then maybeConvert v else maybeConvert { value = v; })
        else
          dag;
      sorted =
        normalized
  ...
  Append = makeProperty {
    ...
    customMap =
      ...
      Append.mkWith { contents = builtins.map fn contents; };
  };
    ...
    lib.mkOptionType {
      ...
      merge =
        ...
        |> builtins.map (
          ...
          {
            inherit file;
            value = if isAppend value then value.contents or [ ] else value;
          }
        )
  ...
  getType =
    ...
    if builtins.isAttrs value then
      if lib.strings.isStringLike value then
        "stringCoercibleSet"
      else if isDag value then
        Dag.tag
      else if Entry.is value then
        Entry.tag
      else if isAppend value then
        Append.tag
      else
      ...
      let
        ...
        commonType = builtins.foldl' (
          ...
          if getType def.value == type then
            ...
            throw "The option `${lib.options.showOption loc}' has conflicting option types:${
  ...
  _tests =
    ...
    in
    {
        ...
        expr =
          let
            data =
              ...
              |> builtins.map (
                x:
                assert lib.assertMsg (x ? _type) "no _type";
                x
              )
              |> builtins.map (foo: {
                inherit foo;
      ...
      "test - freeformType works Append" =
        let
          data =
            (mkAppend [
              {
      ...
      "test - freeformType + options works with dagOf appendOf" = {
        ...
        expected = {
          foo = mkDag {
            a = entryAnywhere (mkAppend [
              "1"
              "1"
            ]);
            b = entryAfter [ "a" "c" ] (mkAppend [ "2" ]);
            c = entryBefore [ "d" ] (mkAppend [ "3" ]);
            d = entryBetween [ "e" ] [ "f" ] (mkAppend [
              "4"
              "X"
            ]);
            e = entryAnywhere (mkAppend [
      ...
      "test - dagEntryOf appendOf merge" = {
        ...
        expected = entryAnywhere (mkAppend [
          1
          2
          3
          4
      ...
      "test dag-module" =
        let
          ...
          module = {
            options.monitorv2 = typed (
              dag.appendOf (dagSubmodule {
                freeformType = freeformConfigType;
