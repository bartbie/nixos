{
  ...
  mappedUniqSubmodule = submoduleType: fn: let
  ...
  hexTypeWith = x:
    types.coercedTo (types.strMatching "#?[0-9a-fA-F]${x}") (s: s |> (lib.removePrefix "#") |> lib.toLower |> (a: "#" + a)) (types.strMatching "#[0-9a-f]${x}");
  hexType =
    hexTypeWith "+";

  hexTypeWithLen = len:
    hexTypeWith "{${builtins.toString len}}";
  ...
  gradientType =
    mappedUniqSubmodule (types.submodule {
      options = {
  ...
  namedEnum = attrs: (
    types.coercedTo
    (types.enum (builtins.attrNames attrs))
    (x: attrs.${x})
    (types.enum (builtins.attrValues attrs))
  );
  namedEnumList = l: l |> lib.imap0 (i: v: lib.nameValuePair v i) |> builtins.listToAttrs |> namedEnum;
