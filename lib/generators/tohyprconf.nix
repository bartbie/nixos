  ...
  configToHyprconf = {
    ...
    */
    renderFields = indent: fields:
      ...
      assert isFields fields; let
        renderFields' = lib.generators.toKeyValue {
          listsAsDuplicateKeys = true;
          inherit indent;
        };

        findFirstPrefixOr = name: default: lib.findFirst (prefix: lib.hasPrefix prefix name) default importantPrefixes;
        no-prefix = "__NO_PREFIX";

        grouped =
          fields
          |> groupByAttrs' (n: _: (findFirstPrefixOr n no-prefix));
    ...
    wrapNamespace = indent: name: s: "${indent}${name} {\n${s
      |> lib.removePrefix "\n"
      |> lib.removeSuffix "\n"}\n${indent}}";
    ...
    */
    ...
    renderDag = indent: name: value:
      assert builtins.isAttrs value; let
      ...
      # assert lib.traceValFn (_: lib.generators.toPretty {} list) true;
        ...
        in
          ...
          then ""
            ...
            join (
              [(renderFields indent (wrap fields))]
              ++ (sections |> builtins.map (renderSection indent name))
            );

    renderSection = indent: name: value:
      assert builtins.isString indent;
      assert builtins.isString name;
      assert isSection value; let
        type =
      ...
      in
        ...
        then wrapNamespace indent name (render (incIndent indent) value)
        else if type == "list"
        then renderList indent name value
        else if type == "append"
        # map to rendering lists
        then renderList indent name value.contents
        else if type == "dag"
        then renderDag indent name value.content
        else if type == "dag-entry"
        then renderFields indent {${name} = value.data;}
        # these last two are for docs purpose
        else if type == "bool"
        then lib.boolToString
        else builtins.toString value;

    render = indent: config:
      assert builtins.isAttrs config; let
