...
{
  filterMapRec =
    leaf-fn: filter-fn: map-fn:
    let
      mark = {
        _bartbie_remove = null;
      };
      marked = x: x ? _bartbie_remove;
    in
    lib.flip lib.pipe [
      (attr.mapAttrsRecursiveCond leaf-fn (p: v: (if (filter-fn p v) then (map-fn p v) else mark)))
  ...
  optionalAttr = name: at: lib.optionalAttrs (attr.hasAttr name at) { ${name} = at.${name}; };
  ...
  bypath = {
    ...
    mapToListCond =
      ...
      let
        ...
        convertToPaths = attr.mapAttrsRecursiveCond cond (
          p: v: {
            path = p;
            value = fn p v;
        ...
        toList = v: if builtins.hasAttr marker v then [ v ] else recurseToList v;
        recurseToList = attr.foldlAttrs (
          acc: _: v:
          acc ++ (toList v)
        ) [ ];
      in
