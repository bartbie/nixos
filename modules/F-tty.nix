{lib, ...}: {
  flake.modules.nixos.base = {
    ...
    just-one-normal-user = config.users.users |> builtins.attrValues |> lib.count (x: x.isNormalUser) |> (x: x == 1);
    owner-user-exists = config.users.users |> builtins.attrNames |> lib.findFirst (x: x == ownername) null |> (x: x != null);
