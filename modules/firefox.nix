      ...
      policies = {
        ...
        ExtensionSettings = let
          mapAddons = let
            ...
            addAddon = name: {version ? "latest", ...} @ opts:
              {
                install_url = "https://addons.mozilla.org/firefox/downloads/latest/${name}/${version}.xpi";
              }
              // defaults
              // lib.filterAttrs (n: _: !(builtins.elem n ["version" "id"])) opts;

            transformOpts = name: opts:
              addAddon name (lib.optionalAttrs (builtins.typeOf opts != "string") opts);
