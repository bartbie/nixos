  ...
  wrapped.hypr = {
    ...
    module = {
      ...
      configDrvs = let
        evalResult =
          ...
          |> (x: x.config)
          |> (x: assert nixonLib.assertions.checkAssertions x.assertions; x);
      in
        evalResult
        |> (x: lib.getAttrs x.out x)
        |> builtins.mapAttrs (
          name: configAttrs: let
    ...
    in {
      single = {
        inherit package;
        wrapper = {
          prependArgs = ["--config" configDrvs.land];
      ...
      finalMapDrv = old: let
        pickAttrs = l: at:
          assert builtins.isList l;
            l
            |> (x: lib.genAttrs x (_: null))
            |> (x: builtins.intersectAttrs x at);
        mkDrv = l:
          assert builtins.isList l;
      ...
      in
        mkDrv [
          ...
          {
            inherit outputs;
            passthru = (old.passthru or {}) // {unwrapped = old;};
