  ...
  in {
    options.workspaceRules = let
      ...
      windowCountFlagsType = lib.mkOption {
        type = coercedToList (types.enum ["t" "f" "g" "v" "p"]);
        apply = x: x |> lib.uniqueStrings |> builtins.sort builtins.lessThan;
        default = [];
      };

      workspaceIdentifierType = types.submodule ({config, ...}: {
        ...
        options = {
          ...
          previous = typedOpt (hyprLib.enumAliased {
            "previous" = ["global"];
            "previous_per_monitor" = ["per_monitor"];
          });
          empty = typedOpt (types.submodule {
            options = {
              m = typed types.bool;
              n = typed types.bool;
            };
        ...
        config = let
          setFields =
            config
            |> (x: builtins.removeAttrs x ["_module" "assertions"])
      ...
      selectorType = types.submodule {
        ...
        options = {
          ...
          w = typedOpt (types.submodule ({config, ...}: {
            options = {
    ...
    config.land.workspace = let
      ...
      getGoto = id: let
        ...
        relAbsToString = prefix: ra:
          if ra.rel != null
          then "${prefix}${relToString ra.rel}"
          else "${prefix}~${toString ra.abs}";
      ...
      in
        {
          ...
          special =
            if builtins.isBool id.special
            then "special"
            else kvToString "special" id.special;
        }.${
          field
        } or (id.${
            field
      ...
      getFilters = let
        nameToString = n:
          if builtins.isBool n
          then hyprLib.toString n
          else if n.starts != null
          then kvToString "s" n.starts
          else kvToString "e" n.ends;

        wcToString = w: let
          flagStr = lib.join "" w.flags;
          countStr =
            if w.exact != null
            then toString w.exact
            else "${toString w.from}-${toString w.to}";
        in "w[${lib.optionalString (flagStr != "") "(${flagStr})"}${countStr}]";

        mapSelector = at: c: fn: lib.optional (at.${c} != null) "${c}[${fn at.${c}}]";
      in
        f: [
          (mapSelector f "r" ({
            from,
          ...
          (builtins.removeAttrs f ["r" "n" "w" "assertions"]
            |> builtins.attrNames
      ...
      |> builtins.map (
        wr:
          [
            ...
            (
              wr
              |> (x: builtins.removeAttrs x ["match" "assertions"])
      ...
      (
        let
          violations =
            ...
            |> lib.mapAttrsToList (
              monitor: entries: "monitor ${monitor}: ${
                entries
                |> builtins.map (r: r.defaultName or (toString r.match.goto.id or "?"))
                |> lib.join ", "
              }"
            );
        in {
