...
in {
  ...
  flake.modules.hypr.api = {config, ...}: {
    options = {
      ...
      layout = lib.mkOption {
        type = types.enum ["dwindle" "master" "scrolling" "monocle"];
        default = "dwindle";
      };
    };
    ...
    config.assertions = [
      {
        assertion = !(builtins.elem config.layout ["scrolling" "monocle"]);
