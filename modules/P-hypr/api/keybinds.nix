...
in {
  flake.modules.hypr.api-keybinds = {
    ...
    normalizeBind = mainMod: {
      ...
      bindType =
        if builtins.elem "m" flags
        then "bindm"
        else if builtins.elem "s" flags
        then "binds"
        else "bind";
      ...
      allMods =
        ...
        |> lib.flatten
        |> lib.unique;
    in "${bindType}:${lib.join "+" allMods}:${keys}";
  in {
    options.keybinds = let
      mkDispatchSubmodule = name: conf: rest:
        mkSubmodule conf ({config, ...}: {
          options = {
            action = lib.mkOption {
          ...
          config.normalize = top-level-config: let
            inherit (config) exec;
          in
            if (exec == null)
            then {inherit (config) action args;}
            else {
              action = "exec";
              args = lib.singleton (
                if (exec.wrap)
                then top-level-config.runCmd exec.cmd
                else exec.dmd
              );
            };

          imports = [rest flake.modules.generic.assertions];
          config.assertions = [
            {
    ...
    in
      lib.mkOption {
        type = types.listOf (mkDispatchSubmodule "keybind" {} ({config, ...}: {
          options = {
            ...
            flags = lib.mkOption {
              type = hyprLib.coercedToList (types.enum [
                "n"
                "e"
                "s"
                "m"
    ...
    config.assertions = nixonLib.assertions.propagateAssertions [
      ...
      (let
        duplicatedBinds =
          config.keybinds
          |> builtins.map (bind: [
            (normalizeBind config.mainMod
              bind)
            (lib.optional (bind.altBehavior != null) (normalizeBind config.mainMod (bind // {mods = bind.mods ++ ["ALT"];})))
          ])
          |> lib.flatten
        ...
        conflictCount =
          duplicatedBinds |> builtins.attrNames |> builtins.length |> builtins.toString;
      in {
