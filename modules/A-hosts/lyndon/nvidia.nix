  ...
  hosts.nixos.lyndon = lib.mkMerge [
      ...
      }:
      {
        ...
        assertions =
          let
            ...
            expected = driver-versions.${if useStableDriver then "stable" else "unstable"};
    ...
    # general
    {
      ...
      boot.extraModprobeConfig =
        ...
        in
        "options nvidia ${lib.join " " options}";
    }
    # prime
    (
      {
        ...
        services.udev.packages =
          let
            ...
            toPciPath =
              ...
              assert (builtins.elemAt split 0) == "PCI";
              "dri/by-path/pci-${domain}:${bus}:${device}.${function}-card";

            pCfg = config.hardware.nvidia.prime;
            igpuPath = toPciPath (if pCfg.intelBusId != "" then pCfg.intelBusId else pCfg.amdgpuBusId);
